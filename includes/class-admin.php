...
class CPO_Admin {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_cpo_get_parent_cats', array( $this, 'ajax_get_parent_cats' ) );
		add_action( 'wp_ajax_cpo_save_parent_cat_grid', array( $this, 'ajax_save_parent_cat_grid' ) );
	}
...
2,456,Venue,Sun Valley Lodge</pre>
	...
	/**
	 * AJAX: Return all root-level (parent = 0) terms for a taxonomy,
	 * ordered by the saved parent grid order, with their representative images.
	 */
	public function ajax_get_parent_cats() {
		check_ajax_referer( 'cpo_sort_nonce', 'nonce' );
		...
		if ( is_wp_error( $root_terms ) ) {
			wp_send_json_error( 'Could not load categories' );
		}

		$saved_order = get_option( 'cpo_parent_grid_order_' . $taxonomy, array() );

		$term_map = array();
		foreach ( $root_terms as $term ) {
			$img_id    = (int) get_term_meta( $term->term_id, '_cpo_grid_image', true );
			$thumbnail = '';
		...
		// Sort by saved order; any unsaved terms go at the end.
		$items = array();
		foreach ( $saved_order as $tid ) {
			if ( isset( $term_map[ $tid ] ) ) {
				$items[] = $term_map[ $tid ];
				unset( $term_map[ $tid ] );
			}
		}
		$items = array_merge( $items, array_values( $term_map ) );
	...
	/**
	 * AJAX: Save the parent-category grid order and any image overrides.
	 * Order stored in a WP option; images stored as term meta.
	 */
	public function ajax_save_parent_cat_grid() {
		check_ajax_referer( 'cpo_sort_nonce', 'nonce' );
		...
		update_option( 'cpo_parent_grid_order_' . $taxonomy, $order );

		foreach ( $images as $tid => $aid ) {
			if ( $aid > 0 ) {
				update_term_meta( $tid, '_cpo_grid_image', $aid );
			} else {
				delete_term_meta( $tid, '_cpo_grid_image' );
			}
		}

		wp_send_json_success( array(
			'message' => sprintf(
				/* translators: %d: number of parent categories reordered */
				__( 'Grid order saved for %d parent categories.', 'custom-portfolio-ordering' ),
				count( $order )
			),
		) );
	}
